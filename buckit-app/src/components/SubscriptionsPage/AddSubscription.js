...
import { Container, Button, Box, Typography, Modal, TextField, FormControl, InputLabel, Input, Select, MenuItem, FormControlLabel, Checkbox } from '@mui/material';
...
export default function AddSubscription(props) {
  ...
  useEffect(() => {
    async function fetchUser() {
      try {
        const user = await props.user_id;
        if (user) {
          setFormData({ ...formData, user_id: user });
        }
      } catch (error) {
        console.error(error);
      }
